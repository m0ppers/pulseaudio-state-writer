...
//       styles.emplace("bluetooth");
//       //   label_.get_style_context()->add_class("bluetooth");
//     } else {
//       styles.erase("bluetooth");
...
//       styles.emplace("muted");
//       styles.emplace("sink-muted");
//       // label_.get_style_context()->add_class("muted");
//       // label_.get_style_context()->add_class("sink-muted");
//     } else {
//       styles.erase("muted");
//       styles.erase("sink-muted");
...
auto waybar::modules::Pulseaudio::update() -> void {
  ...
  Json::Value icons;
  auto const &pulseIcons = getPulseIcon();
  for (int i = 2; i < pulseIcons.size(); i++) {
    icons.append(pulseIcons[i]);
  }
  sink["icons"] = icons;

  Json::Value source;
  source["volume"] = source_volume_;
  source["muted"] = source_muted_;

  result["sink"] = sink;
  result["source"] = source;

  Json::FastWriter writer;
  std::cout << writer.write(result) << std::flush;
}